...
public class Game {
    ...
    public void setDifficulty() {
        ...
        TerminalPrinter.println("====== Dificultad ======\n" +
                "\t1- Facil\n" +
                "\t2- Medio (Por defecto)\n" +
                "\t3- Dificil\n" +
                "\t4- HARDCORE\n");
        int select = TerminalReader.readInt("Seleccione la dificultad (1-4): ");
        int numAttempts = 0;
        switch (select) {
            case 1:
    ...
    public void start() {
        TerminalPrinter.clearLines(4);
        TerminalPrinter.printTitle("AHORCADO");

        do {
            TerminalPrinter.println("\n\tEstado actual: "
                    + this.getWord().getActualStateString() + "\t\tIntentos restantes: " + this.getAttempts());
            TerminalPrinter.println("\tLetras utilizadas: " + this.getUsedLettersString());

            char letter;
            do {
                letter = TerminalReader.readChar("\t\tIntroduzca una letra: ");
